...
data_path = "path_mpc_mpnet"

def save_hash_file(start_goal_hash, save_path='{}/hash.npy'.format(data_path)):
    if path.exists(save_path):
        shutil.copyfile(save_path, save_path+'.bk')
    np.save(save_path, start_goal_hash)

def load_hash_file(save_path='{}/hash.npy'.format(data_path)):
    if path.exists(save_path):
...
class StartGoalGenerator:
    def __init__(self, namespace="", planner_name='voxblox_mpc_mpnet_planner'):
    ...
    def requestPlannerService(self, start, goal, start_goal_hash, start_ori=[0, 0, 0, 1], goal_ori=[0, 0, 0, 1], save_traj=False):
        ...
        try:
            ...
            if resp.success:
                ...
                print(self.data)
                if save_traj:
                    np.save("{}/traj_{}.npy".format(data_path, str(start_goal_hash)[1:-1].replace(", ", "_")),
                            self.data)
                    np.save("{}/start_goal_{}.npy".format(data_path, str(start_goal_hash)[1:-1].replace(", ", "_")),
