...
VoxbloxMPCMPNetPlanner::VoxbloxMPCMPNetPlanner(
    ...
    : nh_(nh),
      ...
      planner_(nh_, nh_private_)
    {
      // For ESDF, copied from rrt implementation
      ...
      planner_srv_ = nh_private_.advertiseService(
          "plan", &VoxbloxMPCMPNetPlanner::plannerServiceCallback, this);
      path_pub_srv_ = nh_private_.advertiseService(
          "publish_path", &VoxbloxMPCMPNetPlanner::publishPathCallback, this);
      ...
      planner_.setRobotRadius(constraints_.robot_radius);
      planner_.setOptimistic(false);

      planner_.setTsdfLayer(voxblox_server_.getTsdfMapPtr()->getTsdfLayerPtr());
      planner_.setEsdfLayer(voxblox_server_.getEsdfMapPtr()->getEsdfLayerPtr());
      ...
      smoother_.setMapDistanceCallback(std::bind(&VoxbloxMPCMPNetPlanner::getMapDistance,
      ...
      loco_smoother_.setMapDistanceCallback(std::bind(
          &VoxbloxMPCMPNetPlanner::getMapDistance, this, std::placeholders::_1));
...
bool VoxbloxMPCMPNetPlanner::publishPathCallback(std_srvs::EmptyRequest& request,
...
void VoxbloxMPCMPNetPlanner::computeMapBounds(Eigen::Vector3d* lower_bound,
...
bool VoxbloxMPCMPNetPlanner::plannerServiceCallback(
  ...
  // Don't in flate in z. ;)
  planner_.setBounds(lower_bound_ - Eigen::Vector3d(kBoundInflationMeters,
                                                kBoundInflationMeters, 0.0),
                upper_bound_ + Eigen::Vector3d(kBoundInflationMeters,
                                                kBoundInflationMeters, 0.0));
  planner_.setupProblem();
  ...
  mav_trajectory_generation::timing::Timer planner_timer("plan/sst");

  bool success =
      planner_.getPathBetweenWaypoints(start_pose, goal_pose, &waypoints);
  planner_timer.Stop();
...
bool VoxbloxMPCMPNetPlanner::generateFeasibleTrajectory(
...
bool VoxbloxMPCMPNetPlanner::generateFeasibleTrajectoryLoco(
    const mav_msgs::EigenTrajectoryPointVector& coordinate_path,
    mav_msgs::EigenTrajectoryPointVector* path) {
...
bool VoxbloxMPCMPNetPlanner::generateFeasibleTrajectoryLoco2(
    const mav_msgs::EigenTrajectoryPointVector& coordinate_path,
    mav_msgs::EigenTrajectoryPointVector* path) {
...
bool VoxbloxMPCMPNetPlanner::checkPathForCollisions(
...
double VoxbloxMPCMPNetPlanner::getMapDistance(
...
bool VoxbloxMPCMPNetPlanner::checkPhysicalConstraints(
