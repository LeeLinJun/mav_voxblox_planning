...
VoxbloxSstPlanner::VoxbloxSstPlanner(const ros::NodeHandle& nh,
  ...
  planner_srv_ = nh_private_.advertiseService(
      "plan", &VoxbloxSstPlanner::plannerServiceCallback, this);
  path_pub_srv_ = nh_private_.advertiseService(
      "publish_path", &VoxbloxSstPlanner::publishPathCallback, this);
  ...
  sst_.setRobotRadius(constraints_.robot_radius);
  sst_.setOptimistic(false);

  sst_.setTsdfLayer(voxblox_server_.getTsdfMapPtr()->getTsdfLayerPtr());
  sst_.setEsdfLayer(voxblox_server_.getEsdfMapPtr()->getEsdfLayerPtr());
  ...
  smoother_.setMapDistanceCallback(std::bind(&VoxbloxSstPlanner::getMapDistance,
  ...
  loco_smoother_.setMapDistanceCallback(std::bind(
      &VoxbloxSstPlanner::getMapDistance, this, std::placeholders::_1));
...
bool VoxbloxSstPlanner::publishPathCallback(std_srvs::EmptyRequest& request,
...
void VoxbloxSstPlanner::computeMapBounds(Eigen::Vector3d* lower_bound,
...
bool VoxbloxSstPlanner::plannerServiceCallback(
  ...
  // Don't in flate in z. ;)
  sst_.setBounds(lower_bound_ - Eigen::Vector3d(kBoundInflationMeters,
  ...
  mav_trajectory_generation::timing::Timer sst_timer("plan/sst");

  bool success =
      sst_.getPathBetweenWaypoints(start_pose, goal_pose, &waypoints);
  sst_timer.Stop();
  ...
  if (visualize_) {
    marker_array.markers.push_back(createMarkerForPath(
        waypoints, frame_id_, mav_visualization::Color::Green(), "sst",
        0.075));
...
bool VoxbloxSstPlanner::generateFeasibleTrajectory(
...
bool VoxbloxSstPlanner::generateFeasibleTrajectoryLoco(
    const mav_msgs::EigenTrajectoryPointVector& coordinate_path,
    mav_msgs::EigenTrajectoryPointVector* path) {
...
bool VoxbloxSstPlanner::generateFeasibleTrajectoryLoco2(
    const mav_msgs::EigenTrajectoryPointVector& coordinate_path,
    mav_msgs::EigenTrajectoryPointVector* path) {
...
bool VoxbloxSstPlanner::checkPathForCollisions(
...
double VoxbloxSstPlanner::getMapDistance(
...
bool VoxbloxSstPlanner::checkPhysicalConstraints(
